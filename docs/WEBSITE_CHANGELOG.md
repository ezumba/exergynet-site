...
**First-party fixes applied to this repository (`exergynet-site`), discovered as a direct consequence of the same forensics:** the wallet `0xbd1e790f6040FA62797671B84a50025a0133109C` — confirmed by this incident's own on-chain checks to have deployed the retired contract above, and already flagged in `CLAUDE.md`'s Pending Credential Rotation list — was live on three pages as the current, trusted "Operator Wallet." The operator has directly confirmed they retain full access to this wallet; it was corrected here because it's flagged for rotation, not because of any confirmed third-party compromise:
...
## 2026-08-28 (later) — Directive 007 resume: operator publish independently verified, 0.2.5 shipped, propagation gate closed (VP Sales Directive 007, resumption)

**Agent/session:** Claude Code session (interactive), operator Seven Ezumba / ExergyNet.

**Context:** the operator reported "OPERATOR GATE 007-A COMPLETE" — npm 0.2.4 published, deprecations applied. Per standing instruction ("independently verify these registry facts rather than trusting this operator report"), none of that report was taken on trust; every claim was re-derived from scratch this pass.

**Genuinely new work this pass:**
- **npm 0.2.4 independently confirmed live**, `latest`, and behaviorally safe — via `npm view`, a clean unpinned `npm install` in a fresh directory, an unpacked tarball inspection, and a live stdio JSON-RPC call against the actual downloaded artifact confirming `exergynet_open_job` fails closed. Deprecation of 0.2.0/0.2.1/0.2.2 (and correct non-deprecation of 0.1.x) confirmed via direct registry queries.
- **Hash discrepancy investigated and resolved**: the locally-built tarball hash recorded in `MCP_RELEASE_PROVENANCE_0.2.4.md` didn't match the npm-published hash. Diagnosed as a Windows-CRLF-vs-published-LF line-ending artifact of this session's environment (`diff --strip-trailing-cr` shows zero content difference), not a real provenance failure. Document corrected with the authoritative npm-published hash.
- **New follow-up shipped: version 0.2.5.** Resolves the four remaining moderate `npm audit` findings (`uuid` via `@solana/web3.js` → `jayson`) with a targeted `package.json` `overrides` entry, explicitly avoiding `npm audit fix --force` (which would have forced a non-functional `@solana/web3.js` downgrade) per instruction. Verified via `npm audit` (0 vulnerabilities), full rebuild, 9/9 regression tests, and a live Solana Mainnet-Beta smoke test. Source committed and pushed to `main` (commit `45f5e29`); npm publication still pending (same credential gap as 0.2.4).
- **Official MCP registry checked and found presenting the retired write-enabled configuration rather than the current fail-closed release**: still serves 0.1.10 metadata that lists `BASE_PRIVATE_KEY` as a *required* field. Root cause identified precisely — the real publish tool (`mcp-publisher`, GitHub-release binary, confirmed current at v1.8.1) requires a GitHub OAuth device-flow under the operator's own `io.github.ezumba` identity, which this session cannot and should not perform.
- **Glama and MCP.so directory listings checked in full** (not just HTTP-status-checked as in a prior pass): both go beyond "stale" — they present the retired autonomous-USDC-approval integration as current, with install instructions telling new users to configure a real signing private key, and zero mention of the security advisory. Both require claiming under the operator's own identity to correct. Smithery confirmed an empty, non-misleading stub. Libraries.io confirmed already correct — it mirrors npm live and needs no action.
- **MCP Vouch rescan attempted, not completed**: the previously-cited scanning site (`mcpvouch.com`, now redirecting to `mcp-registry-dh5.pages.dev`) still shows the stale 71/100 grade-C score from 0.1.10. Its documented self-rescan command, `npx mcpr scan`, could not be run — the `mcpr` package 404s on npm and its linked GitHub repo also 404s. No unverified tool was executed; this is flagged for the operator to confirm the service's current state.
- **Risk-scoping finding, important for messaging discipline going forward**: although Glama/MCP.so still describe the retired write-enabled configuration, a user following their exact install instructions still receives current npm `latest` (0.2.4), whose `exergynet_open_job` fails closed unconditionally regardless of any configured key — so the write path is closed at the npm layer regardless of directory staleness. The residual exposure from stale directories is reputational/trust-accuracy — it does not translate into a live fund-loss path.

**Directive 005 resume gate: now met.** All items gating it — npm safe release published, ordinary install verified, downloaded artifact verified, affected releases deprecated, official-registry staleness clearly documented with root cause, incident docs corrected, zero active retired-address runtime references — are satisfied. The MCP marketing hold tied to "npm serves an unsafe release" can be lifted. This is **not** an unconditional green light for broader claims: outreach must not claim the official MCP registry reflects the current release (it doesn't, and won't until the operator completes a GitHub OAuth step), and must not cite a new MCP Vouch score (none was obtained — the scanning tool itself could not be verified as currently functional).

**No first-party website page changed this pass** — all mutations were to the `exergynet-mcp-server` repository (source) and to this repository's own incident/provenance/advisory/changelog documents.

**Visual QA:** still blocked — four directives in a row now.

## 2026-08-28 (later still) — Directive 008: MCP Distribution Completion + External Recognition Activation

**Agent/session:** Claude Code session (interactive), operator Seven Ezumba / ExergyNet.

**Context:** the operator refined Directive 007's closure into two separate holds — "user-protection hold: closed" but "MCP promotional/analyst hold: partially open" — and issued Directive 008 to close the remaining gap: publish 0.2.5, fix the official registry and directories, find a scanner to replace the non-functional MCP Vouch, complete visual QA, and prepare (not send) analyst outreach materials.

**Genuinely new work this pass:**
- **0.2.5 re-verified via a completely fresh clone** (not the development checkout used to build the fix): 9/9 tests, clean build, `npm audit` → 0 vulnerabilities. `npm pack --dry-run` hash recorded (Windows-CRLF caveat noted, same as every prior release). Not yet published to npm — full operator handoff in `MCP_RELEASE_PROVENANCE_0.2.5.md`.
- **Official MCP registry fix path fully specified**, not just identified as blocked: confirmed the real `mcp-publisher` tool (GitHub-release binary, current v1.8.1), its exact install/login/publish commands (fetched directly from the official docs, not guessed), and confirmed the repo's `server.json` is already publish-ready.
- **Glama's priority raised from P1 to P0** based on a new finding: a same-day search-footprint re-run found Glama as the **#1 organic result for "ExergyNet" across 4 of 6 query variants**, with search engines' AI-generated summaries repeating its retired-write-path framing (autonomous USDC approvals, required private key) nearly verbatim each time. This is the first time this incident chain has measured actual external propagation impact rather than just confirming a listing is stale.
- **MCP.so's real correction path identified, and a costly wrong path avoided**: confirmed the free "Verify ownership via GitHub" button is the correct mechanism — and confirmed the site's `/submit` page is a separate **$39 paid new-listing flow**, explicitly flagged so it isn't mistaken for the fix and paid for unnecessarily.
- **Smithery re-contextualized**: found a "Smithery joins Arcade" banner on the listing, suggesting the platform is mid-acquisition into Arcade.dev — likely explains the persistent empty stub better than "not yet indexed." Recommendation: leave alone.
- **MCP Vouch formally closed as unavailable** (`INDEPENDENT_RESCAN_UNAVAILABLE`), and **a real replacement identified**: Snyk Agent Scan (`github.com/snyk/agent-scan`), confirmed as the actively-maintained successor to Invariant Labs' `mcp-scan` via a GitHub repo-ID redirect trace, not just a name match. Requires the operator's own Snyk account/API token to actually run — not executed this pass, exact command prepared.
- **Structural/functional visual QA performed** across all 10 pages the directive named (see `VISUAL_QA_2026-08-28.md` for full detail and an explicit statement that true screenshot-based QA remains blocked). Found and verified three real, independent-of-screenshots defects:
  - `docs.html` links to `explorer-api.exergynet.org/command.html`, which 404s (as does the domain root).
  - Nearly every page's Discord link is a channel-permalink format that doesn't work as a public join flow for new visitors; only `connect.html` has the correct public invite link.
  - **`mcp.html` documents a first-party, undisclosed, non-functional pseudo-API** — an HTTP+Ed25519-signed "MCP Gateway" whose example code instructs signing a request with a real Solana private key to lock SOL. Verified via a safe probe (no real key/signature used) that the endpoint isn't actually implemented (`POST` → generic 405; `GET` just serves the static doc page). This describes exactly the autonomous-signing pattern the rest of this incident chain has been correcting — found on the company's own site, in a page nobody had previously audited for this. Not corrected unilaterally; flagged as a product decision for the operator.
- **Media/analyst target research completed for real**, replacing the prior pass's placeholder table: 13 independently-verified people/outlets across MCP security, agent memory, and inference-economics beats, each with a real, dated, fetched source — not search-snippet-inferred. Two candidates considered were excluded for failing a currency check (stale employer, out-of-window publish date) rather than included anyway.
- **Evidence packet updated** with a bounded security/release-history section using the operator's exact required framing — no "breach," no "compromised," no universal zero-risk claim.
- **Search footprint re-run** (9 query variants) and compared against the prior baseline — the main finding is the Glama-search-dominance result above; separately, neither the new state/memory narrative canon nor the old sovereign/DePIN/ZK-marketplace framing has meaningfully propagated externally yet (both return effectively zero ExergyNet-specific results), so there is nothing urgent to defend on that specific axis right now.

**No first-party website page was edited this pass.** The two new first-party findings (`mcp.html`, the broken `docs.html` link, the Discord link inconsistency) are flagged in `EXTERNAL_PROPAGATION_CHANGELOG.md` and `MCP_INCIDENT_CLOSURE_2026-08-28.md` for operator decision rather than corrected unilaterally, since the `mcp.html` case in particular is a product/content judgment call (label vs. remove an unimplemented feature description), not a copy-edit.
