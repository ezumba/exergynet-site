...
- **Official MCP registry checked and found not just stale but actively unsafe-looking**: still serves 0.1.10 metadata that lists `BASE_PRIVATE_KEY` as a *required* field. Root cause identified precisely — the real publish tool (`mcp-publisher`, GitHub-release binary, confirmed current at v1.8.1) requires a GitHub OAuth device-flow under the operator's own `io.github.ezumba` identity, which this session cannot and should not perform.
- **Glama and MCP.so directory listings checked in full** (not just HTTP-status-checked as in a prior pass): both are worse than "stale" — they actively present the pre-fix autonomous-USDC-approval behavior as an intended feature, with install instructions telling new users to configure a real signing private key, and zero mention of the security advisory. Both require claiming under the operator's own identity to correct. Smithery confirmed an empty, non-dangerous stub. Libraries.io confirmed already correct — it mirrors npm live and needs no action.
...
- **Risk-scoping finding, important for messaging discipline going forward**: despite the dangerous-looking content on Glama/MCP.so, a user following their exact install instructions still receives current npm `latest` (0.2.4), whose `exergynet_open_job` fails closed unconditionally regardless of any configured key — so the live exploitable path is closed at the npm layer regardless of directory staleness. The residual risk from stale directories is reputational/trust-accuracy, not a live fund-loss path.
