...
| X2 arm, first real run | `X2_REAL_RUN_2026-08-08.md`, `x2_real_run_results.json` | **All 27 dev-set cases, run for real** via isolated Claude subagents (Agent tool, not the API path — see the doc for why). Found and fixed 2 real gate bugs (#15, #16 in the taxonomy) invisible to all prior hand-authored fixtures; after both fixes, 18/27 CONSISTENT, 1 PERMITTED_RECOMMENDATION, 8 non-trivial outcomes -- 3-4 genuine model hedging misses, 3 the gate correctly catching authority questions answered as ASSERTION instead of ACTION_REQUEST (working as designed, not a bug), and 1 open predicate-scoping question. Not holdout, not a full comparator arm -- see the doc's "What this does not establish" section. |
