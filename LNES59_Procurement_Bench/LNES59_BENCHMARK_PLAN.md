...
| X2 arm, first real run | `X2_REAL_RUN_2026-08-08.md`, `x2_real_run_results.json` | **All 27 dev-set cases, run for real** via isolated Claude subagents (Agent tool, not the API path — see the doc for why). 12/27 CONSISTENT, 1 PERMITTED_RECOMMENDATION, 14 non-trivial outcomes -- of which 6 were a real gate bug (#16, string-format mismatch, documented not fixed), a second gate bug (#15) was found and fixed with regression tests, 3-4 were genuine model hedging misses, 3 were the gate correctly catching authority questions answered as ASSERTION instead of ACTION_REQUEST, and 1 is an open predicate-scoping question. Not holdout, not a full comparator arm -- see the doc's "What this does not establish" section. |
