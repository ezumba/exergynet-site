...
**Fixed version:** 0.2.4 (source published to GitHub; npm publication pending — see status note). An intermediate 0.2.3 fixed the same issue but was never published — 0.2.4 supersedes it with additional hardening (see below) before either ever reached npm.
...
2. Upgrade to 0.2.4 or later once it is available on npm (see status below), or remove the package until it is.
